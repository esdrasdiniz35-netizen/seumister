// src/pages/ConfirmeSeuEmail.jsx
...
export default function ConfirmeSeuEmail() {
  ...
  async function handleReenviar() {
    ...
    // Sem e-mail em mãos (ex: alguém deu refresh e perdeu o
    // location.state), não tem como reenviar — manda de volta pro
    // cadastro em vez de fingir que reenviou.
    if (!email) {
      navigate('/cadastro')
      return
    }
    ...
    try {
      // Reenvio real do e-mail de confirmação de cadastro via Supabase.
      // Antes disso essa tela não tinha nenhuma opção de reenvio.
      ...
      if (error) {
        ...
        setMensagem('E-mail reenviado! Confere sua caixa de entrada.')
      }
    } catch (err) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: '14px', fontWeight: '400', color: '#6B7280', lineHeight: '21px', marginBottom: '16px', maxWidth: '360px' }}>
          Clique no link pra ativar sua conta e começar a jogar. Não esquece de checar a caixa de spam.
        </div>
        ...
        >
          Errei o e-mail, voltar ao cadastro
        </button>
